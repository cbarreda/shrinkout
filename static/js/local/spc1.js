...
function vfDrawLine(x,y,iValue){
	...
	objCtx.beginPath();
	objCtx.font = "9px Georgia";
	objCtx.fillStyle = "#000000";		
	objCtx.moveTo(x,y);
	objCtx.lineTo(objCanvas.width,y);
	objCtx.stroke();		
	objCtx.moveTo(x,y);
	objCtx.fillText(iValue,x,y);
...
function vfSetup(arParams,iPoints){
		...
		giUCL = arParams[2];
		giLCL = arParams[3];
		giCL = parseInt((giUCL + giLCL)/2);
		if (giUCL > giHigh)
			giHigh = giUCL;
		if (giLCL < giLow)
			giLow = giLCL;
			
		giOffsetY = parseInt(objCanvas.height / (giHigh - giLow)) ;
		giOffsetX = parseInt((objCanvas.width - giMargin)/iPoints);
...
function vfPos(arAr){
	for (var i = 0; i < 80; i++){
		objCtx.fillText(i,0,ifPointY(i));
	}
}
